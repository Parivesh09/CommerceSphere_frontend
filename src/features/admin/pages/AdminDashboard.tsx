...
export default function AdminDashboard() {
  ...
  const stats = [
    { label: 'Total Revenue', value: '$128,450.00', change: '+14.2%', icon: 'payments', color: 'bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] text-white shadow-glow' },
    { label: 'Saga Transactions', value: '99.98% Success', change: '+0.4%', icon: 'sync', color: 'bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] text-white shadow-glow' },
    { label: 'Total Orders', value: '1,420', change: '+8.1%', icon: 'shopping_bag', color: 'bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] text-white shadow-glow' },
    { label: 'Low Stock SKU Alerts', value: '3 Items', change: 'Action Needed', icon: 'warning', color: 'bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] text-white shadow-glow' },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2 bg-[var(--color-primary)] text-[var(--color-on-primary)] text-xs font-bold rounded-xl shadow-glow hover:brightness-110 transition-colors flex items-center gap-1"
          >
        ...
        {stats.map((stat, idx) => (
          <div key={idx} className="glass-card rounded-2xl p-6 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`material-symbols-outlined text-[22px] p-2 rounded-xl ${stat.color}`}>{stat.icon}</span>
            </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-[var(--color-outline-variant)]">
              {recentTransactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-[var(--color-surface-container-low)] cursor-pointer" onClick={() => navigate(`${ROUTES.ADMIN_ORDERS}/${tx.id}`)}>
